...
const navItems = [
  { id: "overview", label: "查看区", icon: Gauge },
  { id: "actions", label: "配置区", icon: FolderCog },
  { id: "logs", label: "消息通知区", icon: BellRing },
];
...
function App() {
  ...
  const [topbarTitle, setTopbarTitle] = useState("DingTalk");
  ...
  useEffect(() => {
    const syncTopbarTitle = () => {
      ...
      const threshold = 120;
      let nextTitle = "DingTalk";
      let nextTone = "brand";

      for (const region of regions) {
        const rect = region.getBoundingClientRect();
        if (rect.top <= threshold) {
          nextTitle = region.getAttribute("data-region-title") || "DingTalk";
...
function RegionSection({ title, description, children }) {
  const tone =
    title === "查看区" ? "overview" : title === "配置区" ? "config" : title === "消息通知区" ? "notify" : "brand";
...
function TopbarRegion({ title, tone, pendingAction, hasBlockingIssues, onAction }) {
  return (
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-3">
            ...
            <h2 className="truncate text-base font-semibold tracking-tight">
              <span key={title} className="title-swap block">
                {title}
              </span>
            </h2>
          </div>
